...
// Categories 获取所有分类列表（去重，按使用次数排序）
...
func (h *StyleHandler) ToggleFavorite(c *gin.Context) {
	...
	result := h.db.Where("user_id = ? AND style_id = ?", userIDStr, styleID).First(&existing)
	...
	// 未收藏 → 添加收藏
	fav := model.StyleFavorite{UserID: strconv.FormatInt(userID, 10), StyleID: styleID}
	h.db.Create(&fav)
	// 点赞数 +1
	h.db.Model(&model.Style{}).Where("id = ?", styleID).UpdateColumn("likes", gorm.Expr("likes + 1"))
...
func (h *StyleHandler) ListFavorites(c *gin.Context) {
	...
	var favIDs []string
	h.db.Model(&model.StyleFavorite{}).Where("user_id = ?", userIDStr).
...
func (h *StyleHandler) CheckFavorited(c *gin.Context) {
	...
	h.db.Select("style_id").Where("user_id = ? AND style_id IN ?", userIDStr, ids).Find(&favs)
