...
2. **运动提示词**：
   ...
   - **起始状态**：描述镜头开始时的景别、人物初始姿态与表情（使用@角色-角色名称引用）、环境初始状态（使用@场景-场景名称引用）、光线初始效果
...
- shots: 分镜列表，每个分镜包含：
  ...
  - cameraMovement: 运镜方式（含拍摄角度，角度+运镜关键词组合，如："俯视缓慢向前推"、"仰视快速左右摇"、"平视环绕"）
  - dialogue: 角色对白或旁白
  - soundEffect: 音效描述
...
- **shots**: 分镜列表，每个分镜包含：
  - **cameraMovement**: 运镜方式字段需融合拍摄角度和运动方式，格式为"角度+运镜关键词"，如"俯视缓慢向前推"、"仰视快速左右摇"。` + cameraMovementKeywordBlock + `
  ...
  - cameraMovement: 运镜方式（从运镜关键词表中选取，如"向前推"、"环绕"、"轨道跟移"）
  - toneHint: 基调风格提示词
