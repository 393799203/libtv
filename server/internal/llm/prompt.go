...
func BuildAssetImagePrompt(assetType string, description string) string {
	...
	case "scene":
		...
		viewInstruction = "生成场景的四视图，包含正面视图、左侧45度视图、右侧45度视图、背面视图四个视角，以四宫格形式展示上下各两张，完整呈现场景的空间布局、环境氛围和细节元素。"
	case "prop":
		// 道具六视图：正面、背面、左侧、右侧、顶部、底部
		viewInstruction = "生成道具的六视图，包含正面视图、背面视图、左侧视图、右侧视图、顶部视图、底部视图六个视角，以六宫格形式展示，详细展示道具各个角度的外观细节和结构特征。"
	default:
		// 普通图片节点，不添加视图指令
