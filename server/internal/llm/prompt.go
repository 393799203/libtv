...
- **characters**: 剧本中出现的所有角色（2-6个），每个角色的 description 必须是完整的角色三视图文生图提示词，格式如下：
  ...
  【三视图要求】角色设计图，包含正面、侧面、背面三个视角，保持角色特征一致性，适合用于AI图像生成
  这些详细描述是后续AI生图的关键输入，确保角色在不同镜头中保持视觉一致性
- **scenes**: 剧本涉及的所有主要场景（1-5个），每个场景的 description 必须是完整的场景四视图文生图提示词，格式如下：
  ...
  【四视图要求】场景设计图，包含正视角（正面 0°）、左 45° 斜侧视角、右 45° 斜侧视角、背面视角（后视图 180°）四个标准视角，比例 16:9，保持场景特征一致性，适合用于AI图像生成
  这些详细描述是后续AI生图的关键输入，确保场景在不同镜头中保持视觉一致性
- **props**: 剧本中出现的关键道具（1-8个），每个道具的 description 必须是完整的道具六视图文生图提示词，格式如下：
  【基本外观】物品类型、整体尺寸（大/中/小）、主要颜色、材质质感
  【细节特征】表面纹理、装饰图案、特殊标记、磨损痕迹（如有）
  【功能提示】使用状态、发光部件、活动部件、技术细节（如适用）
  【风格调性】时代风格、设计风格、品质感、文化属性
  【六视图要求】道具设计图，包含主视图（正面）、俯视图（顶面）、左视图、右视图、仰视图（底面）、后视图六个标准视角，比例 16:9，保持道具特征一致性，适合用于AI图像生成
...
3. 角色描述必须按照三视图提示词格式详细描述，包含：基本外貌（年龄、性别、身高、体型）、面部特征（发型、发色、眼睛、肤色、五官）、服装细节（上装、下装、鞋子、配饰）、性格气质（表情、姿态、风格）、三视图要求（正面、侧面、背面视角）——这些是后续AI生图的关键输入，确保角色在不同镜头中保持视觉一致性
4. 场景描述必须按照四视图提示词格式详细描述，包含：空间布局、环境氛围、主要元素、风格特征、四视图要求（正视角、左45°斜侧视角、右45°斜侧视角、背面视角），比例16:9——这些是后续AI生图的关键输入，确保场景在不同镜头中保持视觉一致性
5. 道具描述必须按照六视图提示词格式详细描述，包含：基本外观、细节特征、功能提示、风格调性、六视图要求（主视图、俯视图、左视图、右视图、仰视图、后视图），比例16:9——只收录对剧情或视觉有意义的物品，排除普通背景物
6. 镜别和运镜要多样化，避免单调
7. 对白自然，符合角色性格
