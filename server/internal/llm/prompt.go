...
- **scenes**: 剧本涉及的所有主要场景（1-5个），每个场景的 description 必须是完整的场景文生图提示词，**末尾必须包含场景四视图要求和宫格布局**：
  ...
  【场景四视图】生成场景的四视图，包含正面视图、左侧45度视图、右侧45度视图、背面视图四个视角，以四宫格形式展示上下各两张，各宫格之间用1px白边隔离，完整呈现场景的空间布局、环境氛围和细节元素。
...
- **props**: 剧本中出现的关键道具（1-8个），每个道具的 description 必须是完整的道具文生图提示词，**末尾必须包含道具六视图要求和宫格布局**：
  ...
  【道具六视图】生成道具的六视图，包含正面视图、背面视图、左侧视图、右侧视图、顶部视图、底部视图六个视角，以六宫格形式展示，各宫格之间用1px白边隔离，详细展示道具各个角度的外观细节和结构特征。
